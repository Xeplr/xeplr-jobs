...
var _timer = null;
var _ticking = false; // guard against overlapping ticks

/**
 * Start the scheduler loop.
 *
 * config:
 *   models         — { JobType, Job, JobReactivity, JobOccurrence }
...
function startScheduler(config) {
  ...
  tick(); // immediate first pass so we don't wait the full interval

  log.info('xeplr-jobs scheduler started (intervalMs=' + interval + ', maxConcurrent=' + config.maxConcurrent + ')');
