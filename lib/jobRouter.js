 ...
 *   options.overridableInputs — the job input keys a TRIGGER REQUEST may
 *                    replace for one run. Default ['window']. Anything else in
 *                    body.inputs is refused (400), naming the keys.
 */
// WHAT A REQUEST MAY CHANGE ABOUT A SAVED JOB. A job's inputs include its SQL,
// its WHERE clause, its table, procedure and connection — whoever saved the job
// chose those. Letting any caller of /trigger replace them would turn "run this
// job" into "run this SQL on that connection". The period a run covers is the
// thing a caller legitimately picks; widen the list per app, knowingly.
var DEFAULT_OVERRIDABLE_INPUTS = ['window'];

function refusedInputs(inputs, allowed) {
  if (!inputs || typeof inputs !== 'object' || Array.isArray(inputs)) return [];
  return Object.keys(inputs).filter(function(k) { return inputs[k] !== undefined && allowed.indexOf(k) === -1; });
}

function createJobRouter(options) {
  options = options || {};
  var overridable = Array.isArray(options.overridableInputs) ? options.overridableInputs : DEFAULT_OVERRIDABLE_INPUTS;
  ...
  router.post('/jobs/:id/trigger', authMw, async function(req, res) {
    try {
      ...
      var body = req.body || {};
      var refused = refusedInputs(body.inputs, overridable);
      if (refused.length) {
        return respond(res, HTTP.BAD_REQUEST, STATUS.BAD_REQUEST, 'bad_request', {
          message: 'These inputs cannot be overridden by a request: ' + refused.join(', ') + ' — only ' + overridable.join(', ')
        });
      }
  ...
  router.post('/jobs/run', authMw, async function(req, res) {
    try {
      ...
      var refusedByJob = Object.keys(byJob).map(function(id) {
        var r = refusedInputs(byJob[id], overridable);
        return r.length ? id + ': ' + r.join(', ') : null;
      }).filter(Boolean);
      if (refusedByJob.length) {
        return respond(res, HTTP.BAD_REQUEST, STATUS.BAD_REQUEST, 'bad_request', {
          message: 'These inputs cannot be overridden by a request — ' + refusedByJob.join('; ') + ' (only ' + overridable.join(', ') + ')'
        });
      }
...
module.exports.refusedInputs = refusedInputs;
module.exports.DEFAULT_OVERRIDABLE_INPUTS = DEFAULT_OVERRIDABLE_INPUTS;
