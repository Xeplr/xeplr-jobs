...
function createJobRouter(options) {
  ...
  // POST /jobs/run — batch-trigger a set of jobs by id (fire-and-forget)
  //   body: { jobIds: [id1, id2, ...] }
  //   returns: { updatedIds: <triggered>, dataArray: [{ missing: [...] }] }
  router.post('/jobs/run', authMw, async function(req, res) {
    try {
      ...
      var jobs = await models.Job.query().whereIn('id', jobIds).withGraphFetched('jobType');
      var found = new Set(jobs.map(function(j) { return j.id; }));
      var missing = jobIds.filter(function(id) { return !found.has(id); });
      for (var i = 0; i < jobs.length; i++) {
        executeJob(models, jobs[i], { type: 'manual' });
      }
      respond(res, HTTP.OK, STATUS.SUCCESS, 'triggered', {
        updatedIds: jobs.map(function(j) { return j.id; }),
        dataArray: [{ triggered: jobs.length, missing: missing }]
      });
    } catch (err) {
      respond(res, HTTP.SERVER_ERROR, STATUS.SERVER_ERROR, 'server_error', { error: err });
    }
